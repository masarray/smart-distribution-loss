  ...
  return (
    <g aria-label={label}>
      <rect
        ...
        strokeWidth="1.8"
      />
      <path d={`M${x - 7} ${y - 5}L${x + 7} ${y + 5}M${x + 7} ${y - 5}L${x - 7} ${y + 5}`} stroke="currentColor" strokeWidth="1.4" />
      <text x={x} y={y - 15} textAnchor="middle" fill="var(--color-muted-foreground)" fontSize="8.5" fontFamily="var(--font-mono)">
        {label}
      </text>
    </g>
  );
...
function MeterSymbol({ x, y, label }: { x: number; y: number; label: string }) {
  return (
    <g>
      <circle cx={x} cy={y} r="11" fill="var(--color-surface)" stroke="currentColor" strokeWidth="1.7" />
      <text x={x} y={y + 3} textAnchor="middle" fill="currentColor" fontSize="8" fontFamily="var(--font-mono)">M</text>
      <text x={x + 17} y={y + 3} fill="var(--color-muted-foreground)" fontSize="8.5" fontFamily="var(--font-mono)">{label}</text>
    </g>
  );
}

function LoadSymbol({ x, y }: { x: number; y: number }) {
  return <path d={`M${x - 11} ${y - 10}H${x + 11}L${x} ${y + 10}Z`} fill="none" stroke="currentColor" strokeWidth="1.8" />;
}

export function SingleLineDiagram({ selected, onSelect, energised, intensity, gdLossKwh, mvLossKwh }: Props) {
  const flowClass = energised ? (intensity > 0.6 ? "flow-dash" : "flow-dash-slow") : "";

  const zone = (id: AssetId) =>
    cn(
      "cursor-pointer transition-opacity",
      selected === id || selected === "feeder" ? "opacity-100" : "opacity-45 hover:opacity-80",
    );
    ...
    <div className="relative h-full w-full overflow-hidden rounded-lg">
      ...
      >
        ...
        <g color="var(--color-mv)">
          ...
          <path d="M47 93c4-8 8 8 12 0s8-8 12 0" fill="none" stroke="currentColor" strokeWidth="1.5" />
          <text x="80" y="87" fill="var(--color-foreground)" fontSize="12.5" fontFamily="var(--font-display)">GI 150/20 kV</text>
          <text x="80" y="104" fill="var(--color-muted-foreground)" fontSize="9.5">Grid / upstream source</text>
        ...
        <g className={zone("feeder")} onClick={() => onSelect("feeder")} color="var(--color-mv)">
          <path d="M260 93H955" stroke="currentColor" strokeWidth="6" strokeLinecap="round" filter="url(#glow)" />
          <text x="266" y="76" fill="currentColor" fontSize="10.5" fontFamily="var(--font-mono)">BUS 20 kV · PENYULANG GD-01 · 3Φ</text>
          {energised && <path d="M260 93H955" stroke="currentColor" strokeWidth="2.6" className={flowClass} />}
        </g>
        ...
        <g className={zone("spot")} onClick={() => onSelect("spot")} color="var(--color-mv)">
          ...
          {energised && <path d="M390 93V228" stroke="currentColor" strokeWidth="2" className={flowClass} />}
          ...
          <text x="314" y="282" fill="var(--color-foreground)" fontSize="11" fontFamily="var(--font-display)">Spot load TM</text>
        ...
        <g className={zone("tm")} onClick={() => onSelect("tm")} color="var(--color-mv)">
          ...
          {energised && <path d="M600 93V228" stroke="currentColor" strokeWidth="2" className={flowClass} />}
          <rect x="512" y="266" width="176" height="40" rx="6" fill="var(--color-surface-2)" stroke={selected === "tm" ? "var(--color-primary)" : "var(--color-border)"} />
          <text x="524" y="282" fill="var(--color-foreground)" fontSize="11" fontFamily="var(--font-display)">Pelanggan TM</text>
          <text x="524" y="296" fill="var(--color-muted-foreground)" fontSize="9" fontFamily="var(--font-mono)">meter interval · 15 menit</text>
        </g>
        ...
        <g className={zone("gd")} onClick={() => onSelect("gd")} color="var(--color-mv)">
          ...
          {energised && <circle cx="840" cy="188" r="11" fill="var(--color-primary)" opacity="0.18" className="pulse-node" />}
          <text x="870" y="176" fill="var(--color-foreground)" fontSize="11.5" fontFamily="var(--font-display)">TR GD-01</text>
          <text x="870" y="191" fill="var(--color-muted-foreground)" fontSize="9.5" fontFamily="var(--font-mono)">400 kVA · 20/0.4 kV</text>
          ...
          <g color="var(--color-lv)"><CircuitBreaker x={840} y={246} label="QF-LV" /></g>
          <path d="M840 255V274" stroke="var(--color-lv)" strokeWidth="2.6" />
          <path d="M730 274H970" stroke="var(--color-lv)" strokeWidth="6" strokeLinecap="round" filter="url(#glow)" />
          <text x="730" y="260" fill="var(--color-lv)" fontSize="10" fontFamily="var(--font-mono)">BUSBAR 0.4 kV · 3Φ4W</text>
          {energised && <path d="M730 274H970" stroke="var(--color-lv)" strokeWidth="2.4" className={flowClass} />}
          ...
          {[
            ...
            <g key={b.name} color="var(--color-lv)">
              ...
              {energised && <path d={`M${b.x} 274V350`} stroke="currentColor" strokeWidth="1.7" className="flow-dash-slow" />}
              <path d={`M${b.x - 28} 350H${b.x + 28}`} stroke="currentColor" strokeWidth="2.2" />
              <path d={`M${b.x - 18} 350V371M${b.x} 350V371M${b.x + 18} 350V371`} stroke="currentColor" strokeWidth="1.2" opacity="0.8" />
              ...
              <circle cx={b.x + 18} cy="378" r="5" fill="var(--color-surface-2)" stroke="currentColor" />
              <text x={b.x} y="402" textAnchor="middle" fill="var(--color-muted-foreground)" fontSize="9.5" fontFamily="var(--font-mono)">{b.name} · {b.cust} plg</text>
            </g>
          ))}
        </g>
        ...
        <g>
          ...
          <text x="42" y="365" fill="var(--color-foreground)" fontSize="10.5" fontFamily="var(--font-display)">Konvensi SLD</text>
          ...
          <text x="275" y="387" fill="var(--color-muted-foreground)" fontSize="9.5">Upstream → busbar → protection → transformer/load → outgoing feeder</text>
          <text x="42" y="407" fill="var(--color-muted-foreground)" fontSize="9.5">QF = circuit breaker / PMT · M = metering point · dua lingkaran = trafo dua-belitan</text>
          <text x="42" y="423" fill="var(--color-muted-foreground)" fontSize="9.5">Tampilan engineering IEC-style; detail symbol library final mengikuti standard owner / utility drawing practice.</text>
        </g>
      </svg>
    </div>
  );
