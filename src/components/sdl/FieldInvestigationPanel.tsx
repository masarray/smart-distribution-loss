  ...
  return (
    ...
    >
      ...
      <div className="mt-2 grid grid-cols-3 gap-1.5" data-p8-evidence-grid="true">
        <EvidenceCard
          label="Susut puncak"
          value={plan.worstLoss ? `${plan.worstLoss.lossKw.toFixed(3)} kW` : "—"}
          detail={plan.worstLoss?.time ?? "—"}
          dataName="loss"
          numericValue={plan.worstLoss?.lossKw}
          time={plan.worstLoss?.time}
        />
        <EvidenceCard
          label="Loading maks."
          value={plan.worstLoading ? `${plan.worstLoading.loadingPercent.toFixed(1)}%` : "—"}
          detail={plan.worstLoading?.time ?? "—"}
          dataName="loading"
          numericValue={plan.worstLoading?.loadingPercent}
          time={plan.worstLoading?.time}
        />
        <EvidenceCard
          label="Tegangan min."
          value={plan.worstVoltage ? `${plan.worstVoltage.voltagePu.toFixed(3)} pu` : "—"}
          detail={plan.worstVoltage ? `${plan.worstVoltage.busId} · ${plan.worstVoltage.time}` : "—"}
          dataName="voltage"
          numericValue={plan.worstVoltage?.voltagePu}
          time={plan.worstVoltage?.time}
        />
      </div>
      ...
      <div className="mt-2 rounded-md border border-border/40 bg-surface-2/40 px-2 py-1.5 text-[9px] leading-relaxed text-muted-foreground" data-p8-route="true">
        <div className="flex items-center gap-1.5 text-foreground">
          ...
          <span className="font-semibold">Jalur terkait</span>
        </div>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  checked ? "border-success/30 bg-success/5" : "border-border/40 bg-surface-2/35 hover:border-primary/35",
                )}
                data-p8-check-id={item.id}
                data-p8-check-complete={checked ? "true" : "false"}
              >
                <span className={cn("mt-0.5 flex size-3.5 shrink-0 items-center justify-center rounded-sm border", checked ? "border-success bg-success/15 text-success" : "border-border text-transparent")}>
                  <Check className="size-2.5" />
                </span>
                <span className="min-w-0">
                  <span className="block text-[9px] font-semibold leading-snug text-foreground">{item.title}</span>
                  <span className="mt-0.5 block text-[8px] leading-snug text-muted-foreground">{item.evidence}</span>
                </span>
              </button>
            );
          })}
...
function EvidenceCard({ label, value, detail, dataName, numericValue, time }: { label: string; value: string; detail: string; dataName: string; numericValue?: number | undefined; time?: string | undefined }) {
  return (
    <div
      ...
      data-p8-worst={dataName}
      data-p8-worst-value={numericValue == null ? undefined : numericValue.toFixed(6)}
      data-p8-worst-time={time}
    >
...
function ScorePart({ label, value, name }: { label: string; value: number; name: string }) {
  return (
    <span className="rounded bg-surface-2/45 px-1.5 py-1" data-p8-score-part={name} data-p8-score-points={value.toFixed(4)}>
      {label} <span className="numeric text-foreground">{value.toFixed(1)}</span>
    </span>
  );
}
