...
const CUSTOMER_HEADERS = ["customer_id", "bus_id", "phase", "meter_id", "contract_kva", "pf"];
const MEASUREMENT_HEADERS = ["timestamp", "asset_id", "measurement_type", "phase", "value", "unit", "quality"];
const AMI_HEADERS = ["timestamp", "meter_id", "p_kw", "q_kvar", "quality"];
...
function csvObjects(text: string, expectedHeaders: string[], filename: string, errors: string[]) {
  ...
  if (missing.length) {
    errors.push(`${filename}: kolom wajib hilang: ${missing.join(", ")}.`);
    return [] as Record<string, string>[];
  }
...
function numeric(value: string, field: string, context: string, errors: string[], nullable = false): number | null {
  ...
  if (!Number.isFinite(parsed)) {
    errors.push(`${context}: ${field} harus berupa angka${nullable ? " atau kosong" : ""}.`);
    return null;
  }
  return parsed;
}
...
function normalizeTime(raw: string): { time: string; index: number } | null {
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59 || minute % FIELD_INTERVAL_MINUTES !== 0) return null;
  return {
    time: `${String(hour).padStart(2, "0")}:${String(minute).padStart(2, "0")}`,
    index: hour * 4 + minute / FIELD_INTERVAL_MINUTES,
  };
}
...
function parseNetwork(rows: Record<string, string>[], errors: string[]): FieldNetworkElement[] {
  return rows.map((row, i) => {
    const context = `network.csv baris ${i + 2}`;
    const type = row.element_type.trim().toLowerCase() as FieldElementType;
    if (!["source", "line", "transformer"].includes(type)) {
      errors.push(`${context}: element_type harus source, line, atau transformer.`);
    }
    return {
      element_id: row.element_id.trim(),
      element_type: type,
      from_bus: row.from_bus.trim(),
      to_bus: row.to_bus.trim(),
      from_kv: numeric(row.from_kv, "from_kv", context, errors, true),
      to_kv: numeric(row.to_kv, "to_kv", context, errors) ?? 0,
      length_km: numeric(row.length_km, "length_km", context, errors, true),
      r_ohm_per_km: numeric(row.r_ohm_per_km, "r_ohm_per_km", context, errors, true),
      x_ohm_per_km: numeric(row.x_ohm_per_km, "x_ohm_per_km", context, errors, true),
      c_nf_per_km: numeric(row.c_nf_per_km, "c_nf_per_km", context, errors, true),
      r0_ohm_per_km: numeric(row.r0_ohm_per_km, "r0_ohm_per_km", context, errors, true),
      x0_ohm_per_km: numeric(row.x0_ohm_per_km, "x0_ohm_per_km", context, errors, true),
      c0_nf_per_km: numeric(row.c0_nf_per_km, "c0_nf_per_km", context, errors, true),
      max_i_ka: numeric(row.max_i_ka, "max_i_ka", context, errors, true),
      rated_kva: numeric(row.rated_kva, "rated_kva", context, errors, true),
      vk_percent: numeric(row.vk_percent, "vk_percent", context, errors, true),
      vkr_percent: numeric(row.vkr_percent, "vkr_percent", context, errors, true),
      vk0_percent: numeric(row.vk0_percent, "vk0_percent", context, errors, true),
      vkr0_percent: numeric(row.vkr0_percent, "vkr0_percent", context, errors, true),
      pfe_kw: numeric(row.pfe_kw, "pfe_kw", context, errors, true),
      i0_percent: numeric(row.i0_percent, "i0_percent", context, errors, true),
      vector_group: row.vector_group.trim(),
      shift_degree: numeric(row.shift_degree, "shift_degree", context, errors, true),
      s_sc_max_mva: numeric(row.s_sc_max_mva, "s_sc_max_mva", context, errors, true),
      rx_max: numeric(row.rx_max, "rx_max", context, errors, true),
      r0x0_max: numeric(row.r0x0_max, "r0x0_max", context, errors, true),
      x0x_max: numeric(row.x0x_max, "x0x_max", context, errors, true),
    };
  });
}

function parseCustomers(rows: Record<string, string>[], errors: string[]): FieldCustomer[] {
  return rows.map((row, i) => {
    ...
    return {
      ...
      phase,
      meter_id: row.meter_id.trim(),
      contract_kva: numeric(row.contract_kva, "contract_kva", context, errors, true),
      pf,
    };
  });
}

function parseMeasurements(rows: Record<string, string>[], errors: string[]): FieldMeasurement[] {
  const result: FieldMeasurement[] = [];
  rows.forEach((row, i) => {
    const context = `measurements.csv baris ${i + 2}`;
    const normalized = normalizeTime(row.timestamp);
    if (!normalized) {
      errors.push(`${context}: timestamp harus jatuh pada grid 15 menit.`);
      return;
    }
    ...
    result.push({
      ...
      measurement_type: row.measurement_type.trim().toUpperCase(),
      phase: row.phase.trim().toUpperCase(),
      value,
      unit: row.unit.trim(),
      quality: row.quality.trim().toUpperCase() || "UNKNOWN",
    });
  });
  return result;
}

function parseAmi(rows: Record<string, string>[], errors: string[]): FieldAmiPoint[] {
  ...
  rows.forEach((row, i) => {
    ...
    result.push({
      ...
      meter_id: row.meter_id.trim(),
      p_kw: p,
      q_kvar: numeric(row.q_kvar, "q_kvar", context, errors, true),
      quality: row.quality.trim().toUpperCase() || "UNKNOWN",
    });
  });
  return result;
...
export function validateFieldDataset(dataset: FieldDatasetV1): FieldDatasetValidationReport {
  ...
  network.forEach((item) => {
    ...
    if (item.element_type === "source") {
      ...
      for (const [field, value] of [["s_sc_max_mva", item.s_sc_max_mva], ["rx_max", item.rx_max], ["r0x0_max", item.r0x0_max], ["x0x_max", item.x0x_max]] as const) {
        if (value == null || value <= 0) errors.push(`${context}: ${field} wajib dan harus > 0 untuk runpp_3ph.`);
      }
    }
    if (item.element_type === "line") {
      if (!item.from_bus) errors.push(`${context}: line membutuhkan from_bus.`);
      for (const [field, value] of [["length_km", item.length_km], ["r_ohm_per_km", item.r_ohm_per_km], ["x_ohm_per_km", item.x_ohm_per_km], ["r0_ohm_per_km", item.r0_ohm_per_km], ["x0_ohm_per_km", item.x0_ohm_per_km], ["max_i_ka", item.max_i_ka]] as const) {
        if (value == null || value <= 0) errors.push(`${context}: ${field} wajib dan harus > 0.`);
      }
    }
  ...
  ami.forEach((point) => {
    if (!meterSet.has(point.meter_id)) errors.push(`ami.csv: meter_id ${point.meter_id} tidak ditemukan di customers.csv.`);
  });

  const duplicateAmi = new Set<string>();
  const seenAmi = new Set<string>();
  ami.forEach((point) => {
    const key = `${point.meter_id}:${point.index}`;
    if (seenAmi.has(key)) duplicateAmi.add(key);
    seenAmi.add(key);
  });
  ...
  if (missingQ) {
    const metersWithoutPf = customers.filter((customer) => customer.pf == null).map((customer) => customer.meter_id);
    if (metersWithoutPf.length) errors.push(`AMI q_kvar kosong tetapi ${metersWithoutPf.length} meter juga tidak memiliki PF fallback di customers.csv.`);
  ...
  if (amiCoveragePercent < 100) warnings.push(`AMI coverage ${amiCoveragePercent.toFixed(1)}%; physics preview v1 hanya diaktifkan pada 100% P coverage.`);

  const sourceIds = new Set(sources.map((source) => source.element_id));
  const sourcePIndices = new Set(
    measurements
      .filter((m) => sourceIds.has(m.asset_id) && m.measurement_type === "P")
      .map((m) => m.index),
  );
  ...
  const summary: FieldDatasetSummary = {
    networkElements: network.length,
    sources: sources.length,
    lines: lines.length,
    transformers: transformers.length,
    customers: customers.length,
    meters: meterSet.size,
    amiPoints: ami.length,
    amiExpectedPoints,
    amiCoveragePercent,
    sourcePIntervals: sourcePIndices.size,
    sourceMeasurementCoveragePercent: (sourcePIndices.size / FIELD_INTERVALS) * 100,
  };

  const solverReady = errors.length === 0 && customers.length > 0 && amiCoveragePercent === 100;
...
export async function importFieldDataset(files: FileList | File[]): Promise<FieldDatasetImport> {
  ...
  const errors: string[] = [];
  for (const required of REQUIRED_FILENAMES) {
    if (!byName.has(required)) errors.push(`File wajib belum dipilih: ${required}.`);
  }
  if (errors.length) {
    return { dataset: null, report: { valid: false, solverReady: false, errors, warnings: [], summary: emptySummary() }, filenames: array.map((f) => f.name) };
  }

  const [networkText, customerText, measurementText, amiText] = await Promise.all([
    byName.get("network.csv")!.text(),
    byName.get("customers.csv")!.text(),
    byName.get("measurements.csv")!.text(),
    byName.get("ami.csv")!.text(),
  ]);

  const networkRows = csvObjects(networkText, NETWORK_HEADERS, "network.csv", errors);
  const customerRows = csvObjects(customerText, CUSTOMER_HEADERS, "customers.csv", errors);
  const measurementRows = csvObjects(measurementText, MEASUREMENT_HEADERS, "measurements.csv", errors);
  const amiRows = csvObjects(amiText, AMI_HEADERS, "ami.csv", errors);
  ...
  const dataset: FieldDatasetV1 = {
    ...
    network: parseNetwork(networkRows, errors),
    customers: parseCustomers(customerRows, errors),
    measurements: parseMeasurements(measurementRows, errors),
    ami: parseAmi(amiRows, errors),
  };

  const semantic = validateFieldDataset(dataset);
  const report: FieldDatasetValidationReport = {
    ...semantic,
    valid: errors.length === 0 && semantic.valid,
    solverReady: errors.length === 0 && semantic.solverReady,
    errors: [...errors, ...semantic.errors],
  };
  return { dataset, report, filenames: array.map((file) => file.name) };
}
