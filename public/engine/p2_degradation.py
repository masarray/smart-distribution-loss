...
def _build_degraded_view(preset: str):
    ...
    preset_offset = {"good": 1, "typical": 2, "poor": 3}[key]
    measurement_rng = np.random.default_rng(P2_SEED + 900 + preset_offset)
    # Separate deterministic RNG keeps the existing P/phase/voltage measurement
    # sequence unchanged while moving the Q channel into the data-quality layer.
    measurement_q_rng = np.random.default_rng((61850 + 303) + 1200 + preset_offset)
    observed_system = []
    for record in truth["records"]:
        p_noise = 1.0 + measurement_rng.uniform(-cfg["meter_noise_fraction"], cfg["meter_noise_fraction"])
        q_noise = 1.0 + measurement_q_rng.uniform(-cfg["meter_noise_fraction"], cfg["meter_noise_fraction"])
        phase_noise = {
        ...
        observed_system.append({
            ...
            "source_kvar": record["source_kvar"] * q_noise,
            "source_phase_kw": {
