...
def _observed_q_measurements():
    """Measurement-simulator boundary.

    P1 already defines feeder source Q as a Ground Truth measurement channel.
    P2 did not use it in the conventional baseline. P3 exposes only a noisy Q
    meter reading using the same preset noise magnitude. The estimator receives
    the noisy numbers only; it never receives customer true PF/Q.
    """
    cfg = _P2_SESSION["config"]
    preset_offset = {"good": 1, "typical": 2, "poor": 3}[_P2_SESSION["preset"]]
    rng = np.random.default_rng(P3_SEED + 1200 + preset_offset)
    q_obs = np.zeros(P3_INTERVALS, dtype=float)
    for i, record in enumerate(_P1_SESSION["records"]):
        noise = 1.0 + rng.uniform(-cfg["meter_noise_fraction"], cfg["meter_noise_fraction"])
        q_obs[i] = float(record["source_kvar"]) * noise
    return q_obs
